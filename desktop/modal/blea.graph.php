...
<script>
	...
	function load_graph() {
		...
		for (antenna in antennas) {
			...
			graph.addNode(antenna, {
				url: 'plugins/blea/3rdparty/' + icon + '.png',
				antenna: 1,
				x: antennas[antenna]['x'],
				y: antennas[antenna]['y']
			});
		...
		for (eqlogic in eqLogics) {
			haslink = 0;
			graph.addNode(eqLogics[eqlogic]['name'], {
				url: 'plugins/blea/core/config/devices/' + eqLogics[eqlogic]['icon'] + '.jpg',
				antenna: 0
			});
			for (linkedantenna in eqLogics[eqlogic]['rssi']) {
				...
				if (lenghtfactor != 2) {
					;
					haslink += 1;
					graph.addLink(linkedantenna, eqLogics[eqlogic]['name'], {
						isdash: 0,
						lengthfactor: lenghtfactor,
						signal: orisignal
					});
				}
			}
			if (haslink != 0) {
				for (antenna in antennas) {
					...
					if (linked == 0) {
						graph.addLink(antenna, eqLogics[eqlogic]['name'], {
							isdash: 1,
							lengthfactor: -0.1,
							signal: -200
						});
			...
			if (haslink == 0) {
				if ('local' in antennas) {
					graph.addLink('local', eqLogics[eqlogic]['name'], {
						isdash: 1,
						lengthfactor: 0.5,
						signal: -200
					});
					...
					graph.addLink(firstantenna, eqLogics[eqlogic]['name'], {
						isdash: 1,
						lengthfactor: 0.5,
						signal: -200
					});
		...
		$('.bleaRemoteAction[data-action=saveanttenna]').on('click', function() {
			...
			$.ajax({ // fonction permettant de faire de l'ajax
				...
				success: function(data) { // si l'appel a bien fonctionné
					...
					$('#div_alert').showAlert({
						message: 'Positions des antennes sauvées avec succès',
						level: 'success'
					});
					$('#md_modal').dialog('close');
					$('#md_modal').dialog({
